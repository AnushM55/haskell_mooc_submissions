...
prettyPrint size coords = prettyPrint' coords (foldr (++) "" (replicate size ((replicate size '.')++"\n")))
 where prettyPrint' [] board = board
       prettyPrint' (fs:xs) board = prettyPrint' xs ((take ((fst fs-1)*size + (snd fs - 1) + (fst fs -1)  ) board) ++ "Q" ++  (drop ((fst fs-1)*size + (snd fs - 1) + (fst fs)) board))
--------------------------------------------------------------------------------
...
sameDiag (i,j) (k,l) | (k == i && j == l ) = True
		     | otherwise = ((abs (i-k)) == (abs (j-l)) ) && ((div (i-k) (j-l)) > 0 )

sameAntidiag :: Coord -> Coord -> Bool
sameAntidiag (i,j) (k,l) | (k == i && j == l) = True
			 | otherwise = ((abs (i-k)) == (abs (j-l)) ) && ((div (i-k) (j-l)) < 0) 
...
danger (x,y) stack = or (map ((\c1 c2 -> (sameDiag c1 c2) || (sameRow c1 c2) || (sameCol c1 c2) || (sameAntidiag c1 c2) )((x,y))) stack) 
...
prettyPrint2 size list = markDanger (prettyPrint size list) list (1,1)
 where markDanger "" coords init = "" -- empty board
       markDanger (fs:xs) coords (x,y) = case fs of 'Q' -> fs:(markDanger xs coords (if (y == size) then (x+1,1) else (x,y+1)))
       						    '\n' -> fs:(markDanger xs coords (x,y))
						    '.' -> (if (danger (x,y) coords) then '#' else '.'):(markDanger xs coords ((if (y == size) then (x+1,1) else (x,y+1))))
...
fixFirst n ((x,y):xs) | y > n || x > n = Nothing
		      | danger (x,y) xs = fixFirst n ((x,y+1):xs)
...
continue ((x,y):rest) = (x+1,1):(x,y):rest

backtrack :: Stack -> Stack
backtrack (val:(x,y):rest) = (x,y+1):rest
...
step size coords = case (fixFirst size coords ) of Just val -> continue val
...
finish 0 coords = coords
finish n coords | n == (length coords - 1) = (drop 1 coords)
		| otherwise = finish n (step n coords)
