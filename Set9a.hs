...
echo :: String -> String
echo "" = ""
echo (fs:xs) = (echo' (fs:fs:xs))
 where echo' (fs:xs) = case xs of "" -> ""
 				  _ -> xs ++", "++(echo' xs)
       echo' "" = ""
------------------------------------------------------------------------------
...
countValid [] = 0
countValid (fs:xs) = (if (isValid fs ) then 1 else 0) + (countValid xs)
 where isValid digit | (length digit) < 6 = False
 		     | otherwise = ((digit !! 2 ) == (digit !! 4)) || ((digit !! 3 ) == (digit !! 5))
...
repeated [] = Nothing
repeated (fs:[])= Nothing
repeated (fs:xs:[]) = case (fs == xs) of True -> Just fs
					 False -> Nothing
repeated (fs:xs:gs) = case (fs == xs) of True -> Just fs
					 False -> repeated (xs:gs)
...
sumSuccess list = foldr (\value1 value2 -> case (value1,value2) of (Right val1,Right val2) ->  (Right (val1 + val2))
								   (Left _,Right val)     ->  (Right val)
								   (Right val,Left _)   ->  (Right val)
								   _ -> Left "no data") (Left "no data") list
...
data Lock = Lock String | Unlocked String
  deriving (Show , Eq)
...
open code (Lock actual_code ) = case (code == actual_code) of True -> (Unlocked code)
							      False -> (Lock actual_code)
open _ (Unlocked code) = (Unlocked code)

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Unlocked code )= (Lock code)
lock (Lock code) = (Lock code)

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode new_code (Unlocked code) = (Unlocked new_code)
changeCode _ (Lock code) = (Lock code)
...
instance Eq Text where
 (Text word1) == (Text word2) = (filterfunc word1) == (filterfunc word2)
  where filterfunc word = filter (\x -> (not.isSpace $ x)) word
...
compose [] _ = []
compose (x1:fs1) list2 = case (lookup (snd x1) list2 ) of Nothing ->  (compose fs1 list2)
						          Just val -> (fst x1,val):(compose fs1 list2)
...
permute [] target = target

permute list target = permute' 0
 where permute' n | n < (length target) = case (findIndex (\x -> (x == n )) list) of Just val -> (target !! val) : (permute' (n+1))
 		  | otherwise = []
