...
allEqual [x] = True
allEqual (f:s:[]) = (f == s) 
allEqual (f:s:xs) = (f == s) && (allEqual (s:xs) ) 

-- or 

allEqual' :: Eq a => [a] -> Bool
allEqual' fs = (length (nub fs)) == 1
------------------------------------------------------------------------------
...
--
-- Examples:The list of associations of an array in index order.
--   rangeOf [4,2,1,3]          ==> 3
...
longest :: (Ord a) =>  [[a]] -> [a] 
longest list = foldl (\x lis -> case (compare (length lis) (length x)) of EQ -> (case (compare (lis !! 0) (x !! 0)) of EQ -> lis
                                                                                                                       LT -> lis
                                                                                                                       GT -> x)
                                                                          LT -> x
                                                                          GT -> lis ) [] list
...
incrementKey :: ( Num v , Ord k , Eq k ,  Ord v , Eq v ) => k -> [(k,v)] -> [(k,v)]
incrementKey _ [] = []
incrementKey key ((k,v):rest)
    | key == k = (k,v+1) : incrementKey key rest
    | otherwise = (k,v): incrementKey key rest
...
winner scores player1 player2 = case (compare ( Map.findWithDefault 0 player1  scores )
                                              ( Map.findWithDefault 0 player2  scores ) ) of EQ -> player1
                                                                                             LT -> player2
                                                                                             GT -> player1
...
freqs list = foldr (\x acc -> Map.alter  (\maybeval -> case maybeval of Nothing -> Just 1
                                                                        Just val -> Just (val +1)) x acc)  Map.empty list
------------------------------------------------------------------------------
...
transfer from to amount bank
    | amount <= 0 = bank
    | otherwise = case (( Map.member from bank)
                        && (Map.member to bank)
                        && ((Map.lookup from bank) >= Just amount)) of False -> bank
                                                                       True -> Map.alter (\balance -> case balance of Just balance -> Just (balance + amount)) 
                                                                                         to (Map.alter (\balance -> case balance of Just balance -> Just (balance - amount)) from bank )
...
maxIndex arr = foldr (\x y -> if ( (arr ! x) > (arr ! y)) then x else y) (minimum (indices arr)) (indices arr)
