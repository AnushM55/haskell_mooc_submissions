...
countdownPrint n | (n == 0) = do putStrLn (show n)
		 | (n < 0) = do 
		    putStrLn (show n)
		    countdownPrint (n+1)
		 | otherwise = do
...
isums :: Int -> IO Int
isums val = isums' val 0
 where isums' n runningsum | (n == 0) = return 0
 			   | otherwise = do
			   userval <- getLine
			   val <- (readIO userval :: IO Int)
			   print (runningsum + val)
			   rest <- (isums' (n-1) (runningsum + val))
			   return (val + rest)
