...
fmap2 f  values =  (f <$>) <$> values

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f values = (fmap2 f) <$> values
...
instance Functor Result where
  ...
  fmap f (MkResult val) = MkResult ( f val)
...
instance Functor List where
 fmap f Empty = Empty
 fmap f (LNode val Empty) = LNode (f val) Empty
 fmap f (LNode val next_node) = LNode (f val) (fmap f next_node)
...
instance Functor TwoList where
 ...
 fmap f (TwoNode val1 val2 next_node) = TwoNode (f val1) (f val2) (f <$> next_node)
...
count val list = foldl (\x y -> (x + (if y == val then 1 else 0))) 0 list
...
inBoth list1 list2 = foldl (\x y -> (x ++ (foldl (\a b -> (if (b==y) then (b:a) else a)) [] list2))) [] list1
...
instance Foldable List where
  foldr _ initial_value Empty = initial_value
  foldr f initial_value (LNode val next_node) = f val (foldr f  initial_value next_node)
------------------------------------------------------------------------------
...
instance Foldable TwoList where
  foldr _ initial_value TwoEmpty = initial_value
  foldr f initial_value (TwoNode val1 val2 next_node) = f val1 ( f val2 (foldr f initial_value next_node))
...
instance Functor Fun where
 fmap f (Fun function) = Fun (f <$> function)
...
sumTree :: Monoid m => Tree m -> m
sumTree (Leaf )= mempty Leaf
sumTree (Node val left right) =(sumTree left) <>  val <> (sumTree right)
