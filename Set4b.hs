...
countHelper val prev = prev + (maybe 1 (\x -> 0) val)
...
maxHelper val prev =  max val prev
...
slHelper val prev = ( val + (fst prev) , 1 + (snd prev))
...
concatHelper :: [a] -> [a] -> [a]
concatHelper val prev =  val ++ prev 
...
largestHelper val prev = if (val >= maxi) then (if (val > maxi) then val:[] else val:prev) else prev
                        where maxi = (maximum prev)
...
lastHelper val x = case x of Nothing -> Just val
                             _ -> x
