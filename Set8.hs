...
dotAndLine = Picture f
	where  f (Coord x y) = case (x,y) of (3,4) -> white
	                                     (_,8) -> pink
					     _ -> black
------------------------------------------------------------------------------
...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = (Color (avg r1 r2) (avg g1 g2) (avg b1 b2) )
    where avg col1 col2 = div (col1+col2) 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine func (Picture f1) (Picture f2) = Picture f
  where f (Coord x y) = func (f1 (Coord x y)) (f2 (Coord x y))
...
rectangle x0 y0 w h = Shape f
 where f (Coord x y) = ((x >= x0 )&& (y >= y0))  && ( ((x-w) < x0) && (y-h) < y0)
------------------------------------------------------------------------------
...
union (Shape f1) (Shape f2) = Shape f
 where f (Coord x y) = (f1 (Coord x y))  || (f2 (Coord x y))  

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape f
 where f (Coord x y) = (f1 (Coord x y))  && ( (f2 (Coord x y)) /= True)  
------------------------------------------------------------------------------
...
paintSolid color (Shape func) (Picture basefunc) = Picture f
 where f (Coord x y) = case (func (Coord x y)) of True -> color
 						  False -> (basefunc (Coord x y))
------------------------------------------------------------------------------
...
paint (Picture patternfunc) (Shape shapefunc) (Picture basefunc) = Picture f
 where f (Coord x y) = case (shapefunc (Coord x y)) of True -> (patternfunc (Coord x y))
 						       False -> (basefunc (Coord x y))
...
instance Transform Fill where
  apply (Fill color) _ = Picture func
   where func (Coord x y) = color
...
instance Transform Flip where
  ...
  apply FlipX (Picture f) = Picture (f . (\(Coord x y) -> (Coord (-1*x) y)))
  apply FlipY (Picture f) = Picture (f . (\(Coord x y) -> (Coord x (-1*y))))
------------------------------------------------------------------------------
...
blendColors :: Color -> Color -> Color -> Color -> Color -> Color
blendColors (Color r1 g1 b1) (Color r2 g2 b2) (Color r3 g3 b3) (Color r4 g4 b4) (Color r5 g5 b5) = (Color (avg r1 r2 r3 r4 r5) (avg g1 g2 g3 g4 g5) (avg b1 b2 b3 b4 b5) )
    where avg col1 col2 col3 col4 col5 = div (col1+col2+col3+col4+col5) 5

instance Transform Blur where
  apply Blur (Picture func) = Picture blur_func
   where blur_func (Coord x y) = blendColors (func (Coord x y)) (func (Coord (x-1) y)) (func (Coord (x) (y-1))) (func (Coord (x) (y+1))) (func (Coord (x+1) y))
------------------------------------------------------------------------------
...
instance Transform BlurMany where
  ...
  apply (BlurMany times) pic =  apply Blur (apply (BlurMany (times - 1)) pic)
------------------------------------------------------------------------------

-- Here's a blurred version of our original snowman. See it by running
--   render blurredSnowman 400 300 "blurred.png"

blurredSnowman = apply (BlurMany 30) exampleSnowman
