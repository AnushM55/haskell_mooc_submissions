...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if (measure a) == (maximum $ map measure [a,b]) then a else b
...
mapMaybe f arg  = case arg of Nothing  -> Nothing
                              Just arg -> Just . f $ arg
------------------------------------------------------------------------------
...
firstHalf str
    | (odd  (length str)) =  take (( div (length str) 2)+1) str
...
capitalizeFirst (f:xs) = (:) (toUpper f) xs

capitalize :: String -> String
capitalize sentence = unwords . map capitalizeFirst $ words $ sentence 
------------------------------------------------------------------------------
...
whileRight check x = case (check x) of Left xs -> xs
                                       Right xs -> (whileRight check xs)
...
joinToLength len list = filter (\x -> length(x) == len) [  x++y |x <- list , y <- list ]
...
------------------------------------------------------------------------------
-- Ex 11: remember the lectureParticipants example from Lecture 2?
-- We used a value of type [Either String Int] to store some measurements
-- that might be missing.
-- Implement the function sumRights which sums all non-missing measurements in a list like this.
--
...
sumRights lis = case lis of [] -> 0
                            (Left x:xs) ->  ( sumRights xs)
                            (Right x:xs) -> x + (sumRights xs)
------------------------------------------------------------------------------
...
multiCompose fs init_val = case fs of [] -> init_val
                                      fs -> ( (head fs) ((multiCompose (tail fs) init_val)))
------------------------------------------------------------------------------
...
multiApp finalfunc xs value = finalfunc  (map' xs)
                              where map' [] = []
                                    map' (g:gs) = g value : map' gs 
...
interpreter ops = helper ops 0 0 

helper :: [String] -> Integer -> Integer -> [String]
helper [] _ _ = []
helper (f:fs) stateX stateY  = case f of "printX" -> (show stateX) : helper fs stateX stateY
                                         "printY" -> (show stateY) : helper fs stateX stateY
                                         "up" -> helper fs stateX (stateY+1)
                                         "down" -> helper fs  stateX (stateY - 1)
                                         "left" -> helper fs (stateX - 1) stateY
                                         "right" -> helper fs (stateX + 1) stateY
                                         _ -> helper fs stateX stateY
