...
appendAll string [] = do (return  ())
appendAll string (fs:xs) = do
 val  <- newIORef fs
 modifyIORef string (++fs)
 appendAll string xs
...
swapIORefs x y = do
 xval <- readIORef x
 yval <- readIORef y
 modifyIORef x (\x -> yval)
 modifyIORef y (\y -> xval)
...
hFetchLines content = do
 cond <- (hIsEOF content)
 if (cond == False) then do
  line <- (hGetLine content)
  rest <- (hFetchLines content)
  return ( line : rest )
 else
  return []
------------------------------------------------------------------------------
...
hSelectLines h [] = do return []
hSelectLines h lines = do
 fetched_lines <- (hFetchLines h)

 final_lines <- (hSelectlines' fetched_lines lines)
 return final_lines
  where hSelectlines' :: [String] -> [Int] -> IO [String] 
        hSelectlines' list [] = do return []
        hSelectlines' list (fs:xs) = do
	 rest_of_list <- (hSelectlines' list xs)
	 return ((list !! (fs-1)) : rest_of_list)
...
get_bool (a,b,c) = a
get_string (a,b,c) = b
get_newstate (a,b,c) = c
interact' :: ((String,st) -> (Bool,String,st)) -> st -> IO st
interact' f state = do
 line <- getLine
 let triplet = (f (line,state))

 let input_str = (get_string triplet)
 putStrLn input_str
 let cond = (get_bool triplet)
 
 let new_state = (get_newstate triplet)
 if (cond == True) then do
  final <- (interact' f new_state)
  return final
 else do
  return new_state
